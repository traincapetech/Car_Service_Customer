...
export const marketplaceApi = {
  /**
   * Fetch current partner workshop profile (status, verification, coordinates)
   * GET /api/v1/partner/workshop/profile
   */
  async getPartnerProfile() {
    const response = await apiRequest("/partner/workshop/profile", {
      method: "GET",
      requiresAuth: true,
    });
  ...
  /**
   * Initiate Razorpay payment for a lead opportunity
   * @param {number|string} opportunityId - Opportunity ID
   * @param {string} [idempotencyKey] - Unique idempotency key
   */
  async initiateRazorpayPayment(opportunityId, idempotencyKey) {
    const key = idempotencyKey || `RZP-INIT-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
    const response = await apiRequest(`/partner/payments/initiate`, {
      method: "POST",
      body: {
        opportunityId: Number(opportunityId),
  ...
  /**
   * Verify Razorpay payment signature and atomically claim lead opportunity
   * @param {number|string} paymentId - Internal payment ID
   * @param {Object} verifyData - { razorpayOrderId, razorpayPaymentId, razorpaySignature }
   */
  async verifyRazorpayPayment(paymentId, verifyData) {
    const response = await apiRequest(`/partner/payments/${paymentId}/verify-razorpay`, {
      method: "POST",
      body: {
  ...
  /**
   * Fetch workshop active floor jobs
   * @param {string} [status] - Optional WorkshopJobStatus filter
   */
  async getWorkshopJobs(status) {
    const query = status ? `?status=${encodeURIComponent(status)}` : "";
    const response = await apiRequest(`/workshops/jobs${query}`, {
      method: "GET",
      requiresAuth: true,
    });
    return response?.data || [];
  ...
  /**
   * Fetch single workshop job details
   * @param {number|string} id - Workshop job ID
   */
  async getWorkshopJob(id) {
    const response = await apiRequest(`/workshops/jobs/${id}`, {
      method: "GET",
      requiresAuth: true,
    });
    return response?.data;
   ...
   * @param {number|string} id - Workshop job ID
   * @param {string} targetStatus - New WorkshopJobStatus
   * @param {string} [notes] - Optional status notes
   */
  async updateJobStatus(id, targetStatus, notes = "") {
    const response = await apiRequest(`/workshops/jobs/${id}/status`, {
      method: "PUT",
      body: {
        targetStatus,
        notes,
  ...
  /**
   * Get customer tracking information by booking or service request ID
   * @param {number|string} id - Booking or Service Request ID
   */
  async getCustomerTracking(id) {
    const response = await apiRequest(`/customer/tracking/${id}`, {
      method: "GET",
      requiresAuth: true,
    });
    return response?.data;
  ...
  /**
   * Cancel customer service request
   * @param {number|string} id - Service Request ID
   */
  async cancelServiceRequest(id) {
    const response = await apiRequest(`/service-requests/${id}/cancel`, {
      method: "PUT",
      requiresAuth: true,
    });
    return response?.data;
